...
- **[`docs/engineering-protocol.md`](docs/engineering-protocol.md)** — the operating protocol (how work is done: net-first, phase-and-commit, pre-clear gate, lock-by-construction). Entry point; points at canonical sources, doesn't duplicate them.
...
- **Value / gain (unknown ≠ 0)**: a set's value or gain is consumed ONLY via the null-aware functions in `src/utils/portfolio.js` (`setValueProvenance().amount`, `setGain`, `setROI`, `portfolioValue/Gain/ROI`, `groupRollup`) and rendered via `valueDisplay.js` (`formatValueCell`). Unknown value is `null` → "—" / excluded, NEVER `$0`. Never write your own `asNumber(s.totalValue) || asNumber(s.currentValue) * qty` or `value - paid` at a consumer site — that re-opens the falsy-zero class. (Cost/spent stays inclusive: `$0` adds `$0`.)
  - **0 = unknown, for value, single-sourced.** A stored `0` value and an absent field are the SAME — both unknown (no set is genuinely worth `$0`). The 0→unknown coalescing lives in ONE place: `valueAmount()` in `src/utils/value.js`, used by `rawSetValue` (set-level) and the SetDetailPanel per-copy path. Never treat a `0` value as genuine, and never bypass `valueAmount` on a value read — `value.zero-unknown.test.js` guards it. This is **VALUE-only**: for cost, `$0` can be genuine (GWP), so do NOT apply `valueAmount` to cost (`toValue`/`normalizeAmount` stay general and keep a `0`).
