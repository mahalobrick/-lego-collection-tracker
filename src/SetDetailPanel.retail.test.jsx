...
// Render and return the "vs. Retail" StatBox text (label + value), or null if absent.
function renderVsRetail(setNumber, extra = {}) {
  const item = { setNumber, condition: "new", quantity: 1, entries: [], ...extra };
  act(() => root.render(<SetDetailPanel item={item} onClose={() => {}} />));
  const box = [...container.querySelectorAll("div")].find(d => d.firstChild?.textContent === "vs. Retail");
  return box ? box.textContent : null;
}

describe("SetDetailPanel retail chip — browser-observable (DOM-leaf)", () => {
  // Label is "Retail" — unified with the row column / "vs. Retail" stat / Retail Value card (Phase 3b).
  ...
  it("scenario 3 — no retail anywhere: chip shows \"—\"", () => {
    expect(renderPanel("11111-1")).toBe("Retail —");
  });
  ...
  it("scenario 4 — manual msrp only (no caches): chip shows the figure, tagged 'manual' (Phase 3a)", () => {
    ...
    expect(renderPanel("33333-1", { msrp: 4.99 })).toBe(`Retail ${money(4.99)}manual`);
  });

  it("scenario 5 — Brickset present + manual msrp: Brickset wins, no manual tag", () => {
    seedBrickset("10300-1", 199.99);
    expect(renderPanel("10300-1", { msrp: 4.99 })).toBe(`Retail ${money(199.99)}`);
  });
});

describe("SetDetailPanel vs. Retail % — reads the resolved ladder, not a raw field (Phase 3b)", () => {
  ...
  // regression repointed vs-Retail at item.retailPrice, the guard `retailPrice && …` would be
  // falsy here and the StatBox would vanish — this test fails.
  it("Brickset-only set (no raw retailPrice) still gets a correct vs. Retail %", () => {
    seedBrickset("10300-1", 100);
    // value known (totalValue) + cost > 0 → vs. Retail renders. Market 150 vs retail 100 → +50.0%.
    const txt = renderVsRetail("10300-1", { totalValue: 150, totalPaid: 90 });
    expect(txt).toBe("vs. Retail+50.0%");
  });
});
