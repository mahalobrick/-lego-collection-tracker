...
| Medium | `TEST-1` | Zero test infrastructure — no test runner, no tests, no CI | [Error Handling, Observability & Testing](#area-errors-testing) | moderate | CONFIRMED → Medium |
...
| Medium | `DATA-4` | Sync depends entirely on the monkey-patched setItem; any bypass writes data that never syncs | [Data & State](#area-data-state) | trivial | — |
...
| Medium | `SEC-GAP-2` | No tooling enforces the 'every /api handler authenticates first' invariant | [Security Touchpoints (× docs/security.md)](#area-security-recon) | moderate | — |
...
### 3.4 The `localStorage.setItem` patch (`src/main.jsx:17-28`)
...
- **The two highest-value invariants are enforced only by convention, not by tooling.** security.md §4e pattern 1 ("every `/api` handler authenticates in its first 3 lines") and §4f pattern 1 ("stop shipping proxy handlers without an auth check") are prose. The original APISEC-1 bug was precisely a copy-paste scaffold that omitted the auth line 8×; nothing in the repo prevents the 10th proxy from repeating it. There is no `.claude/rules/` dir and no `PreToolUse` hook (`.claude/settings.local.json` has none). This is the strongest candidate for hard enforcement. (See finding `SEC-GAP-2`, `hardEnforce=true`.)
...
CLAUDE.md is guidance, not enforcement. These invariants should be enforced mechanically:

- **OBS-2** — 111 unguarded localStorage.setItem calls — QuotaExceededError causes silent data loss (`src/main.jsx:21 (patched setItem); src/MyCollection.jsx:799 (purchase write inside empty catch); 111 setItem call sites total`)
- **DATA-4** — Sync depends entirely on the monkey-patched setItem; any bypass writes data that never syncs (`src/main.jsx:17-28 (localStorage.setItem patch) and SYNC_SKIP_KEYS at :11-16`)
- **SEC-GAP-2** — No tooling enforces the 'every /api handler authenticates first' invariant (`.claude/ (no rules dir / no PreToolUse hook); pattern lives only as prose in docs/security.md §4e.1 & §4f.1`)
- **B6** — No enforced money type — 215 asNumber() call sites rely on author discipline (`src/MyCollection.jsx (61), src/WantedList.jsx (64), src/BudgetDashboard.jsx (59), +SetDetailPanel/AppSettings/PurchaseDetailPanel/WatchDetailPanel; canonical def src/utils/formatting.js:1-3`)
...
- [ ] **TEST-1** — Zero test infrastructure — no test runner, no tests, no CI — _moderate_ · `package.json (scripts: build, dev only); no *.test.* files; no .github/ directory`
      ↳ Add vitest + an 'npm test' script + a minimal GitHub Action running it on PR. Cost is low (Vite-native, no extra config) and unblocks the targeted tests below.
...
- [ ] **DATA-4** `🔒` — Sync depends entirely on the monkey-patched setItem; any bypass writes data that never syncs — _trivial_ · `src/main.jsx:17-28 (localStorage.setItem patch) and SYNC_SKIP_KEYS at :11-16`
      ↳ Add a path-scoped .claude/rules entry (or PreToolUse hook) forbidding capture/reassignment of localStorage.setItem outside src/main.jsx and requiring new persistent data keys to use the bl/brickEconomy prefix so the patch and clearLocalUserData both cover them. Document the contract in a comment block referenced by the registry from DATA-1.
...
- [ ] **SEC-GAP-2** `🔒` — No tooling enforces the 'every /api handler authenticates first' invariant — _moderate_ · `.claude/ (no rules dir / no PreToolUse hook); pattern lives only as prose in docs/security.md §4e.1 & §4f.1`
      ↳ Add a path-scoped .claude/rules entry (or a PreToolUse hook on Edit/Write to api/*.js) asserting that any new/edited api/*.js handler — except api/_*.js helpers — imports and calls requireAuth (or authenticate) before any process.env.*_KEY read or external fetch. Cheap CI alternative: a grep test that fails if an api/ handler references a *_API_KEY/SCRAPERAPI_KEY env var without also referencing requireAuth/authenticate.
...
- [ ] **OBS-4** — reconcileOnSignIn treats a fetch failure as a silent 'local wins' decision — _moderate_ · `src/App.jsx:94-95`
      ↳ Distinguish 'cloud empty/404' (safe to proceed) from 'fetch errored' (network/5xx). On a genuine error, surface a toast and either keep syncReadyRef false (defer push) or retry before deciding, rather than silently enabling push.
