...
**SECRET-4 — Production deploy runs on Clerk *development*-instance keys** · 🟡 Medium · Confirmed · **ACCEPTED RISK** (ops)
Browser-console pass on the prod deploy (`lego-app-rust.vercel.app`) shows Clerk's **"development keys"** warning — prod is using a Clerk *development* instance (`pk_test_…`/`sk_test_…`) rather than a **production instance** (`pk_live_…`/`sk_live_…`). Dev instances are explicitly not for production: relaxed bot/abuse protection, lower limits, a shared dev Frontend API, and the visible dev banner. Ties to [AUTH-1](#category-2--authentication) (`authorizedParties`/`APP_ORIGIN`). **Config/ops, not code.**

**Decision (2026-05-31): ACCEPTED RISK.** BrickLedger is a **personal, single-user app** with no custom domain and no Clerk production instance provisioned. The dev-instance posture (relaxed abuse protection, shared dev Frontend API, dev banner) is a real downgrade *for a public, multi-user deployment* — but it is not a live threat to a single operator's own data on a low-traffic deploy. The mitigation cost (a custom domain + Clerk production instance) is not justified until the app actually goes public. This is an accepted trade-off, not an unseen gap: the diagnosis above stands; we are choosing not to remediate yet.

**On-ramp verified (so this stays a value-swap, not a code hunt):** Clerk keys are fully **env-sourced** with nothing hardcoded — repo-wide grep for `pk_test_`/`sk_test_`/`pk_live_`/`sk_live_` and `clerk.*`/`*.accounts.dev` literals returns **zero** hits in source. Client: `@clerk/react`'s `<ClerkProvider>` (`src/main.jsx:13`) auto-reads `VITE_CLERK_PUBLISHABLE_KEY` from `import.meta.env` (resolved by `@clerk/shared`); no `publishableKey` literal in code. Server: `api/_auth.js:30` reads `CLERK_SECRET_KEY` from `process.env` only. Both values live in Vercel env / `.env.local`, never in tracked source. **Net effect: going public is an env-value swap + redeploy with NO code change** — except the OAuth step below.
...
5. **Provision production OAuth credentials per provider** — *required because Google social login is enabled (see above)*: create a Google Cloud OAuth client, set the authorized redirect URI Clerk gives you, and paste the client ID/secret into the Clerk production instance's Google connection. (Skip only if social login is later disabled.)
6. **Swap the keys in Vercel env** (Production scope): set `VITE_CLERK_PUBLISHABLE_KEY` → `pk_live_…` and `CLERK_SECRET_KEY` → `sk_live_…` from the production instance. Update `APP_ORIGIN` to the new domain so [AUTH-1](#category-2--authentication)'s `authorizedParties` matches.
7. **Redeploy** (env changes need a fresh build for `VITE_*` to embed).
8. **Verify a real sign-in** on the live domain: dev banner gone, email/password + Google sign-in succeed, `/api/sync` round-trips. Because keys are env-sourced (verified above), **steps 6–7 are the only "swap" — no source edit** beyond the one-time `APP_ORIGIN` env value; step 5 (Google OAuth creds) is the one genuinely new provisioning task.
